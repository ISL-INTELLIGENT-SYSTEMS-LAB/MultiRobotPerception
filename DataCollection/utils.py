'''
This file provides some supporting functions to the network_relation script
...
## NOT CURRENTLY IN USE, USED FOR AN OLD IDEA
class Node:
...
def update_correspondence(cols, rows, correspondence, first_file, second_file):
	if not correspondence[first_file].isnull().all():
		for i in cols:
			...
			mat_idx = correspondence.loc[correspondence[first_file] == i].index.item()
			correspondence.at[mat_idx, second_file] = rows[lst_idx].item()
