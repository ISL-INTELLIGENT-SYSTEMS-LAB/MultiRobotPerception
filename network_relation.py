...
def getDataFrames(root, files):
        ...
        ## Intialize the list of dataframes
        dataframes = []

        ## Go through each file in the files, create dataframes, and append each to the dataframes list
        for file in files:
                dataframes.append(get_data(root, file))

        ## Obtain the largest number of detected objects across all the dataframes
        ...
        ## Finally, we return the finished list of dataframes
...
def network_relation(root, files): ## Powerhouse method which manipulates data for relation
        ...
        dataframes, max_objects_detected = getDataFrames(root, files)
        ...
        print(correspondence_df) # Print the final correspondence DataFrame
